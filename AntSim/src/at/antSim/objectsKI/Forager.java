...
public class Forager extends Ant {
	...
	public void reactSpecific(GhostPhysicsObject ghostPhysicsObject) {
		...
		if (ghostPhysicsObject.getType().equals("positionLocator")) { // ant is
																		// inside
																		// a
																		// positionLocator

			// System.out.println(this +
			// " inside a positionLocator and lockedLocator == " +
			// lockedLocator);

			PositionLocator locator = (PositionLocator) parentingEntities
					.get(ghostPhysicsObject);
			positionLocators.increaseCount(locator); // make sure
														// positionLocator
														// remains/is being
														// added to list of all
														// PositionLocators ant
														// is currently in

			if (locator != null) {
				if (lockedLocator == null) { // only register ant for one
												// locator at a time
					if ((locator.getTarget().getObjectType()
							.equals(ObjectType.FOOD) && foodtransport < maxFoodTransport)
							|| (locator.getTarget().getObjectType()
									.equals(ObjectType.ENEMY) && foodtransport == 0)
							|| (locator.getTarget().getObjectType()
									.equals(ObjectType.HIVE) && foodtransport > 0)) {

						if (locator.getTarget().getObjectType()
								.equals(ObjectType.HIVE)
								&& foodtransport > 0) {
							// System.out.println(this +
							// " is trying to enter hive locator");
						}

						if (locator.getTarget().getObjectType()
								.equals(ObjectType.ENEMY)) {
							movementManager
									.addMovementEntry(
											(DynamicPhysicsObject) locator
													.getTarget().physicsObject,
											new Wait(
													(DynamicPhysicsObject) locator
															.getTarget().physicsObject,
													physicsObject,
						...
						if (locator.entryPossible(this)) {
							if (locator.getTarget().getObjectType()
									...
									&& foodtransport > 0) {
								// System.out.println(this +
								// " was allowed entry into hive locator");
							}

							lockedLocator = locator;
							locator.registerAnt(this); // ant will be added to
														// active ants in
														// locator
							movementManager
									.addMovementEntry(
											...
											new MoveToTarget(
													physicsObject,
													(ReadOnlyPhysicsObject) locator.physicsObject,
													Globals.ANT_SPEED));
						} else { // entry not possible -> either add to waiting
									// list by calling locator.registerAnt or do
									// something else
							if (locator.getTarget().getObjectType()
									.equals(ObjectType.HIVE)
									&& foodtransport > 0) {
								// System.out.println(this +
								// " was told to wait for entry into hive locator");
							}
							...
							locator.registerAnt(this); // ant will be added to
														// waiting ants in
														// locator
							movementManager.addMovementEntry(
									physicsObject,
									new Wait(physicsObject,
											(ReadOnlyPhysicsObject) locator
													.getTarget().physicsObject,
											Globals.ANT_SPEED));
							// movementManager.addMovementEntry(physicsObject,
							// new MoveToTarget(physicsObject, physicsObject,
							// Globals.ANT_SPEED));
						}

					}
				} else if (locator.getTarget().getObjectType()
						.equals(ObjectType.HIVE)
						&& foodtransport == 0) {

				} else {
					if (!locator.containsActiveAnt(this)) {
						if (locator.registerAnt(this)) { // try to get access ->
															// if allowed, ant
															// will be added to
															// actives, so
															// method will not
															// be called again
							movementManager
									.removeLastMovementEntry(physicsObject); // remove
																				// wait
																				// entry
																				// (which
																				// forces
			...
			pheromones.increaseCount((Pheromone) parentingEntities
					.get(ghostPhysicsObject));
			...
			if (p != null && p.getDirection() != null)
				movementManager.addMovementEntry(physicsObject,
						new MoveInDirection(physicsObject, p.getDirection(),
								Globals.ANT_SPEED));
		}
	}
...
//		System.out.println("static: " + staticPhysicsObject.getType());
		...
		if (lockedLocator != null) {
			if (staticPhysicsObject
					.equals(lockedLocator.getTarget().physicsObject)) { // ant
																		// ran
																		// into
																		// lockedLocator
				...
				if (lockedLocator.getTarget().objectType == ObjectType.FOOD) {
					if (foodtransport < maxFoodTransport) {
						...
						Food food = (Food) parentingEntities
								.get(staticPhysicsObject);
						carryMoreFood(food.harvest()); // amount of food which
														// cannot be carried by
														// ant is put back into
														// food resource

						if (!Entity.entities.contains(food)) { // food has been
																// deleted cause
																// its
																// foodstacks
																// reached 0
						...
						if (lockedLocator != null) {
							...
							movementManager
									.addMovementEntry(
											...
											new MoveToTarget(
													physicsObject,
													(ReadOnlyPhysicsObject) lockedLocator.physicsObject,
													Globals.LOCKIN_SPEED));
						} else {
							if (movementManager.getTopMovementMode(
									physicsObject).getType() == MovementModeType.TARGET) {
								movementManager
										.removeLastMovementEntry(physicsObject); // whatfor?
							}
							movementManager
									.addMovementEntry(
											physicsObject,
											new MoveToTarget(
													physicsObject,
													(ReadOnlyPhysicsObject) hive.physicsObject,
													Globals.ANT_SPEED));
						}
					} else {
						if (movementManager.getTopMovementMode(physicsObject)
								.getType() != MovementModeType.DODGE) {
							movementManager
									.removeLastMovementEntry(physicsObject); // whatfor?
							movementManager
									.addMovementEntry(
					...
					if (foodtransport > 0) {
						...
						if (movementManager.getTopMovementMode(physicsObject) != null) {
							if (movementManager.getTopMovementMode(
									physicsObject).getType() != MovementModeType.BASIC) {
								movementManager
										.removeLastMovementEntry(physicsObject); // So
																					// he
																					// doesnt
																					// stand
																					// at
	...
	public void reactSpecific(DynamicPhysicsObject dynamicPhysicsObject) {
		...
		if (Entity.parentingEntities.get(dynamicPhysicsObject).objectType == ObjectType.ENEMY) {
			Enemy enemy = (Enemy) Entity.parentingEntities
					.get(dynamicPhysicsObject);
			enemy.fight(attack);
			fight(enemy.attack);
		}
	}
