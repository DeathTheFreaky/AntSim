...
in vec3 toLightVector; //vector from vertex to light source in world space
...
void main(void) {
	...
	vec3 diffuse = brightness * lightColor;	//get final lighting color for a pixel (scalar - brightness is multiplied with each component of vector x,y,z)
	...
	outColor = vec4(diffuse, 1.0) * textureColor + vec4(finalSpecular, 1.0) ; //pixels color is created by applying diffuse lighting on the texture color and adding the reflection from specular lighting
